...
@pytest.mark.parametrize(
    "settings,error",
    [
        ({"ssp_templates_file": 'ssp_data.h5'}, OSError),
    ],
)
def test_DSPSSingleSedModeler_templates_not_found(settings, error):
    """
    ...
    ----------
    settings: dict
        dictionary having "ssp_templates_file" as keyword and not existing path to trigger OSError
    error: built-in type
        OSError
    Returns
    -------
...
def test_DSPSPopulationSedModeler_templates_not_found(settings, error):
    """
    Test if ssp templates filepath exists.
...
def test_DSPSPhotometryCreator_templates_not_found(settings, error):
    """
    Test if ssp templates filepath exists.
