import sys
import numpy as np
#sys.path.insert(0,"/global/homes/s/schmidt9/DESC/software/PhotoZDC1/src")
...
def grabdata(healpix=9816, frac=100):
    """
    Parameters:
    -----------
    hpix: int
    ...
    bands = ['u', 'g', 'r', 'i', 'z', 'y']
    for band in bands:
        columns.append(f"mag_{band}_lsst")
    data = cat.get_quantities(columns, native_filters = [f'healpix_pixel=={healpix}'],filters=['mag_i_lsst<25.'])
...
def generateflow(data=None, hpix=None, frac=20):
    ...
    mag_idx = [data.columns.get_loc(col) for col in ["mag_u_lsst","mag_g_lsst","mag_r_lsst","mag_i_lsst","mag_z_lsst","mag_y_lsst"]]
    column_idx = 0
    sharpness = 10
