...
returns a ModelHandle, and wraps the run method
"""
...
from rail_lib_gp_comp import rail_lib_gp_comp


class GalaxyPopulationModeler(Modeler):
    r"""
    Derived class of Modeler for creating a galaxy population model from library of galaxy_modelling components.
    ...
    def __init__(self, args, comm=None):
        ...
        #allowed_modelling_components = rail_lib_gp_comp.list_available_components()
        ...
        if rail_lib_gp_comp.check_implemented_components(self.config.luminosity_function_type) is False:
            raise KeyError("The {} component is not implemented in the library"
                           .format(self.config.luminosity_function_type))
    ...
    def fit_model(self):
        """
        Produce a model for the galaxy population from which samples can be drawn.
        ...
        -------
        model: ModelHandle
            ModelHandle storing the galaxy population model
        """
        self.run()
        self.finalize()
