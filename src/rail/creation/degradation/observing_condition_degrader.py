...
class ObsCondition(Degrader):
    ...
    def _validate_obs_config(self):
        ...
        if not os.path.exists(self.config["mask"]):
            raise ValueError("The mask file is not found: "
                             + self.config["mask"])
            ...
            for key in self.config["map_dict"]:
                ...
                if key == "nYrObs":
                    ...
                    if key in ["airmass", "tvis"]:
                        ...
                        if not (isinstance(self.config["map_dict"][key], str) or isinstance(self.config["map_dict"][key], float)):
                    ...
                    # band-dependent keys
                        ...
                        for band in self.config["map_dict"][key].keys():
                            ...
                            if not (isinstance(self.config["map_dict"][key][band], str) or isinstance(self.config["map_dict"][key][band], float)):
