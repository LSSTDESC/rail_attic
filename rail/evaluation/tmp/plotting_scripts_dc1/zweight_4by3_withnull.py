#File to make plots of all zweight photo-z's calculated from
#Dritan's script.
#Feb 15, 2018
import sys,os
import numpy as np
...
def main():
    ...
    outputfile = "ZWEIGHT_POINT_STATS_ALL.out"
...
#first, calculate the sigmas for each code
        szpzobj = pmet.EvaluatePointStats(zweight[i],sz[i],imag[i],
                                          imagcut=imagcut)
        ...
        ax.scatter(szpzobj.sz_magcut,szpzobj.pz_magcut,marker='.',c='b',
                   edgecolor='none',lw=1,label=tmplabel)
        ...
        plt.yticks(np.arange(0.,1.99,0.5))
    ...
    plt.savefig("ZWEIGHT_szpz_threecolumn_12codes_navy.jpg",format='jpg')
    #plt.savefig("szpz_fourcolumn_11codes_navy.pdf",format='pdf')
    #plt.savefig("ZWEIGHT_szpz_fourcolumn_11codes_navy.ps",format='ps')
    plt.show()
